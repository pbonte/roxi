...
impl TripleStore {
    ...
    pub fn decode_rule(rule: &Rule) -> String{
        ...
        let decoded_body : String = rule.body.iter().map(|t|Self::decode_triples(&[t.clone()])).collect();
...
mod tests {
    ...
    fn test_store() {
        ...
        for i in 0..max_depth{
            let rule = Rule{head: Triple{s:VarOrTerm::new_var("s".to_string()),p:VarOrTerm::new_term("http://test".to_string()),o:VarOrTerm::new_term(format!("U{}", i+1)), g: None},
                body: Vec::from([Triple{s:VarOrTerm::new_var("s".to_string()),p:VarOrTerm::new_term("http://test".to_string()),o:VarOrTerm::new_term(format!("U{}", i)), g: None}])};
            rules.push(rule);
        }

        let content =  Vec::from([Triple{s:VarOrTerm::new_term("sTerm".to_string()),p:VarOrTerm::new_term("http://test".to_string()),o:VarOrTerm::new_term("U0".to_string()), g: None}]);
        let mut rules_index = RuleIndex::new();
        for rule in rules.iter(){
            rules_index.add_ref(rule);
        }
        let mut triple_index = TripleIndex::new();
        content.into_iter().for_each(|t| triple_index.add(t));
        let query = Triple{s:VarOrTerm::new_var("s".to_string()),p:VarOrTerm::new_term("http://test".to_string()),o:VarOrTerm::new_term(format!("U{}", max_depth)), g: None};

        let mut store = TripleStore{rules:Vec::new(), rules_index , triple_index, reasoner: Reasoner{ } };
